...
TEST_CASE("Controllo Urti"){
   ...
   /*SUBCASE("controllo dynamics == dynamicsAnimated") {
    ...
    ball::r2(100);
    ball::radius(10);
    float y0[10] = {-195.f, -195.f,-194.9f, -194.9f, -100.f, 0.f,0.f, 100.f, 194.9f, 194.9f};//200.f, 200.f};
    float theta0[10] = {-0.7f,0.7f, -0.6f, 0.6f , 1.2f, -0.4f,0.4f, 1.2f, -0.6f, 0.6f};// -0.7f, 0.7f};
    float finalY[10] = {-86.2f,0.,89.f,-90.f,-17.f,-89.f,89.f,-192.f,90.56f,-89.f};
    //float finalM[] = {};
    for (int i{0}; i < 10; i++) {
    ball test(0.,y0[i],std::tan(theta0[i]));
    std::cout << "entrato in test con y0 = " << y0[i] << " e con theta0 = " << theta0[i] << '\n';
     test.ballDynamics();
     //std::cout << "eseguita dynamics" << '\n';
    CAPTURE(y0[i]);
    CAPTURE(test.getY());
    CAPTURE(finalY[i]);
   ...
   /*SUBCASE("urti funzionanti(r2 > r1)") {
    ...
    ball::r2(200);
    ball::radius(1e-11);
    float y0[6] = {0.f,0.f,100.f,100.f,195.f,195.f};
    float theta0[6] = {-0.3f,0.9f,-0.7f,0.7f,-1.2f,1.2f};
    float finalY[6] = {60.f,-60.f,57.7f,142.2f,22.8f,32.8f};
    //float finalM[] = {0.f};
    ...
    for (int i{0}; i < 6; i++) {
     ...
     test.ballDynamics();
     CAPTURE(y0[i]);
     std::cout << "valori miei= " << test.getY() << " valori Paolo:" << finalY[i] << '\n';
     CHECK(std::abs(test.getY()- finalY[i]) < 5.);
     //CHECK(std::abs(test.getM()- finalM[i]) < 5.);
